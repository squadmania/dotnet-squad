...
{
    public sealed class PlayerDiedPayloadParser : IPayloadParser<PlayerDiedPayload>
    {
        private static readonly Regex PayloadRegex = new Regex(@"\[DedicatedServer](?:ASQSoldier::)?Die\(\): Player:(.+) KillingDamage=(?:-)*([0-9.]+) from ([A-z_0-9]+) caused by ([A-z_0-9-]+_C)");
        
        public LogMessageType LogMessageType => LogMessageType.SquadTrace;

        public PlayerDiedPayload? ParseTyped(
            string value
        )
        {
